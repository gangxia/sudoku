...
public class OpenSudokuImportTask extends AbstractImportTask {
	...
	protected void processImport() throws SudokuInvalidFormatException {
		try {
			...
			if (mUri.getScheme().equals("content")) {
				...
				java.net.URI juri;
				juri = new java.net.URI(mUri.getScheme(),
						mUri.getSchemeSpecificPart(), mUri.getFragment());
	...
	private void importV2(XmlPullParser parser) throws XmlPullParserException,
			IOException, SudokuInvalidFormatException {
			...
			if (eventType == XmlPullParser.START_TAG) {
				...
				if (lastTag.equals("folder")) {
					...
					long created = parseLong(
							parser.getAttributeValue(null, "created"),
							System.currentTimeMillis());
					importFolder(name, created);
				} else if (lastTag.equals("game")) {
					importParams.clear();
					importParams.created = parseLong(
							parser.getAttributeValue(null, "created"),
							System.currentTimeMillis());
					importParams.state = parseLong(
							parser.getAttributeValue(null, "state"),
							SudokuGame.GAME_STATE_NOT_STARTED);
					importParams.time = parseLong(
							parser.getAttributeValue(null, "time"), 0);
					importParams.lastPlayed = parseLong(
							parser.getAttributeValue(null, "last_played"), 0);
	...
	private void importV1(XmlPullParser parser) throws XmlPullParserException,
			IOException, SudokuInvalidFormatException {
		int eventType = parser.getEventType();
		String lastTag = "";
