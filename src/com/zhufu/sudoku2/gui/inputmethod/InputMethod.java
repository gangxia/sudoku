...
package com.zhufu.sudoku2.gui.inputmethod;
...
import com.zhufu.sudoku2.game.Cell;
import com.zhufu.sudoku2.game.SudokuGame;
import com.zhufu.sudoku2.gui.SudokuBoardView;
import com.zhufu.sudoku2.gui.inputmethod.IMControlPanelStatePersister.StateBundle;
...
public abstract class InputMethod {
	...
	// TODO: I should not have mPrefix for fields used in subclasses, create
	// proper getters
	...
	public View getInputMethodView() {
		if (mInputMethodView == null) {
			...
			View switchModeView = mInputMethodView
					.findViewById(R.id.switch_input_mode);
			...
			switchModeButton.getBackground().setColorFilter(
					new LightingColorFilter(Color.CYAN, 0));
	...
	/**
	 * This should be called when activity is paused (so InputMethod can do some
	 * cleanup, for example properly dismiss dialogs because of WindowLeaked
	 * exception).
	 */
	public void pause() {
		onPause();
	...
	/**
	 * Gets abbreviated name of input method, which will be displayed on input
	...
	/**
	 * Called when cell is selected. Please note that cell selection can change
