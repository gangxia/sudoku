...
import com.zhufu.sudoku.gui.exporting.FileExportTask;
import com.zhufu.sudoku.gui.exporting.FileExportTask.OnExportFinishedListener;
import com.zhufu.sudoku.gui.exporting.FileExportTaskParams;
import com.zhufu.sudoku.gui.exporting.FileExportTaskResult;
...
public class SudokuExportActivity extends Activity {
	...
	private static final String TAG = SudokuExportActivity.class
			.getSimpleName();
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		if (intent.hasExtra(EXTRA_FOLDER_ID)) {
			mExportParams.folderID = intent.getLongExtra(EXTRA_FOLDER_ID,
					ALL_FOLDERS);
		...
		String timestamp = DateFormat.format("yyyy-MM-dd", new Date())
				.toString();
		if (mExportParams.folderID == -1) {
			...
			if (folder == null) {
				Log.d(TAG, String.format(
						"Folder with id %s not found, exiting.",
						mExportParams.folderID));
				finish();
				return;
			}
	...
	private void exportToFile() {
		...
		if (!sdcard.exists()) {
			Toast.makeText(SudokuExportActivity.this,
					R.string.sdcard_not_found, Toast.LENGTH_LONG);
			finish();
		}
	...
	private void startExportToFileTask() {
		mFileExportTask
				.setOnExportFinishedListener(new OnExportFinishedListener() {
					...
					public void onExportFinished(FileExportTaskResult result) {
						...
						if (result.successful) {
							Toast.makeText(
									SudokuExportActivity.this,
									getString(
											R.string.puzzles_have_been_exported,
											result.file), Toast.LENGTH_SHORT)
									.show();
						} else {
							Toast.makeText(SudokuExportActivity.this,
									getString(R.string.unknown_export_error),
	...
	//
	// mFileExportTask.setOnExportFinishedListener(new
	// OnExportFinishedListener() {
	//
	// @Override
	// public void onExportFinished(FileExportTaskResult result) {
	...
	// Toast.makeText(SudokuExportActivity.this, "no way to share folder",
	// Toast.LENGTH_SHORT).show();
	// }
	// } else {
	// Toast.makeText(SudokuExportActivity.this, "not successful.",
	// Toast.LENGTH_LONG).show();
	// }
	//
	// finish();
