package com.zhufu.sudoku.gui.importing;
...
import com.zhufu.sudoku.db.SudokuImportParams;
import com.zhufu.sudoku.db.SudokuInvalidFormatException;
import com.zhufu.sudoku.game.FolderInfo;
import com.zhufu.sudoku.gui.ImportSudokuActivity;
import com.zhufu.sudoku.utils.Const;

/**
 * To add support for new import source, do following:
 * 
 * 1) Subclass this class. Any input parameters specific for your import should
 * be put in constructor of your class. 2) In {@link #processImport()} method
 * process your data source (parse file or maybe download data from some other
 * source) and save puzzles by calling {@link #importFolder(String, boolean)}
 * and {@link #importGame(String)} methods. Note that <code>importFolder</code>
 * must be called first, otherwise <code>importGame</code> doesn't know where to
 * put puzzles. 3) Add code to {@link ImportSudokuActivity} which creates
 * instance of your new class and passes it input parameters.
 * 
...
public abstract class AbstractImportTask extends
	...
	protected void onPostExecute(Boolean result) {
		if (result) {
			...
			if (mFolderCount == 1) {
				Toast.makeText(
						mContext,
						mContext.getString(R.string.puzzles_saved, mFolder.name),
						Toast.LENGTH_LONG).show();
			} else if (mFolderCount > 1) {
				Toast.makeText(
						mContext,
						mContext.getString(R.string.folders_created,
								mFolderCount), Toast.LENGTH_LONG).show();
			}
	...
	private Boolean processImportInternal() {
		...
		Log.i(Const.TAG,
				String.format("Imported in %f seconds.", (end - start) / 1000f));
	...
	/**
	 * Starts appending puzzles to the folder with given <code>name</code>. If
	 * such folder does not exist, this method creates new one.
	 * 
	 * @param name
	 */
	 ...
	 * 
	 * @param game
	 *            Fields to import (state of game, created, etc.)
	 * @param data
	 *            Data to import.
	 */
	protected void importGame(SudokuImportParams pars)
			throws SudokuInvalidFormatException {
		if (mDatabase == null) {
			throw new IllegalStateException("Database is not opened.");
		}
